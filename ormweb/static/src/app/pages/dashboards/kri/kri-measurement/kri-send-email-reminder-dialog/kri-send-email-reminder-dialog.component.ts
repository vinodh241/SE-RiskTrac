...
export class KriSendEmailReminderDialogComponent implements OnInit {
  ...
  showApprovedCommentBox: boolean = false;
  ...
  ngOnInit(): void {
    ...
    this.minimize()
    this.showBulkApprovalCommentBox();
    this.showBulkRejectionCommentBox();
  }
  ...
  showBulkRejectionCommentBox() {
    this.alldata.forEach((data: any) => {
      if (data.KRI_Status == "Rejected" && data.IsSaved == true) {
        this.rejectedKRI.push(data);
      }
    })
    // console.log("Rejected KRI: ", this.rejectedKRI)
    if (this.rejectedKRI.length > 0) {
      this.showRejectionCommentBox = true
    } else if (this.rejectedKRI.length > 0 && this.approvedKRI.length > 0) {
      this.showRejectionCommentBox = true;
      this.showApprovedCommentBox = true;
    } else {
      this.showRejectionCommentBox = false;
      this.defineKRIForm.get('bulkRejectionTextareaData')?.disable();
    }
    // console.log("show rejected comment box: ", this.showRejectionCommentBox);
    return this.showRejectionCommentBox;
  }

  showBulkApprovalCommentBox() {
    this.alldata.forEach((data: any) => {
      if (data.KRI_Status == "Approved" && data.IsSaved == true) {
        this.approvedKRI.push(data);
      }
    })
    // console.log("Approved KRI: ", this.approvedKRI);
    if (this.approvedKRI.length > 0) {
      this.showApprovedCommentBox = true
    } else if (this.rejectedKRI.length > 0 && this.approvedKRI.length > 0) {
      this.showRejectionCommentBox = true;
      this.showApprovedCommentBox = true;
    } else {
      this.showApprovedCommentBox = false
    }
    // console.log("show approved comment box: ", this.showApprovedCommentBox);

    return this.showApprovedCommentBox;
  ...
  submit() {
    this.submitted = true;
    //  console.log("this.defineKRIForm.invalid", this.defineKRIForm.invalid);
    //  console.log("this.defineKRIForm", this.defineKRIForm);
