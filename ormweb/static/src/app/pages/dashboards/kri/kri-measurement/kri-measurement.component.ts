...
@Component({
    selector: 'app-kri-measurement',
    templateUrl: './kri-measurement.component.html',
    styleUrls: ['./kri-measurement.component.scss']
})
export class KriMeasurementComponent implements OnInit {
    ...
    displayedColumns: string[] = [
        ...
        'upload', 'action', 'sendEmail', 'checkerComments'
    ];
    ...
    sendIndividualEmailReminder(metricId: number) {
        ...
        confirm.afterClosed().subscribe(result => {
            ...
            if (result) {
                this.kriService.sendIndividualRemainder(data).subscribe(res => {
                    next:
                    if (res.success == 1) {
                        this.saveSuccess(res.message);
                    } else {
    ...
    saveReviewerDetails(): void {
        ...
        this.dataSource.data.forEach((metric: any) => {
            ...
            sample.push({
                ...
                "commentBody": metric.CommentData,
                "isApproved": metric.IsReviewedData,
                "isSaveReviewerData": (this.isReviewedStatus || metric.CommentData != '' || metric.CommentData != null || metric.CommentData != undefined) ? true : false
            })
        })
        let data = {
            "KRIReviewData": [
    ...
    showComments(element: any): void {
        let comments = ""
        if (element.CommentData != null) {
            comments = element.CommentData ? (element.CommentData.filter((x: any) => x.IsVisible) || []) : [];
        }
