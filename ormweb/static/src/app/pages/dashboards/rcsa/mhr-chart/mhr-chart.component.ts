...
export class MhrDashboardComponent implements OnInit {   
    ...
    ngOnInit(): void { 
      ...
      this.dashboardService.gotYearQuater.subscribe((value) => {          
        ...
        setTimeout(()=>{  
            this.listData = this.dashboardService.master.filter((item: any) => item.Quater === this.currentQuarter);
      ...
      // let currentQuarter = "Q" + (Math.floor((currentDate.getMonth() - 3) / 3) + 2) + "-" + currentDate.getFullYear().toString().substr(-2);         
      // this.listData = this.dashboardService.master.filter((item: any) => item.Quater === currentQuarter);
      //   this.setChart(this.viewall);
      //  }, 2000)
      // this.dashboardService.getMhrSchedule();
      // this.inherentData = this.dashboardService.Mhrmaster.inherent.slice(0, 5);
      // this.residualData = this.dashboardService.Mhrmaster.residual.slice(0, 5);
