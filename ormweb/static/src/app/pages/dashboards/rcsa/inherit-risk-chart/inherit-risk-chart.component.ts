...
export class InheritRiskChartChartComponent implements OnInit {
  ...
  handleData() {
    ...
    // console.log('handleData-this.prevQuarter::', this.prevQuarter);
    // console.log('handleData-this.currentQuarter::', this.currentQuarter);
  ...
  createChart() {
    ...
    var prevLowMidRisk = this.prevQuarterData.filter(
      (ele: any) => (ele.InherentRiskRating == "Low" || ele.InherentRiskRating == "Medium")
    )
    var curRedHigh = [];
    var curRedHighNew = [];
    for (var i = 0; i < this.prevQuarterData.length; i++) {
      for (var y = 0; y < this.curQuarterData.length; y++) {
        if (
          ...
          if (
            (this.prevQuarterData[i].InherentRiskRating == 'Low' || this.prevQuarterData[i].InherentRiskRating == "Medium") &&
            (this.curQuarterData[y].InherentRiskRating == 'Severe' || this.curQuarterData[y].InherentRiskRating == 'Catastrophic' || this.curQuarterData[y].InherentRiskRating == 'High') && this.prevQuarterData[i].SLNO == this.curQuarterData[y].SLNO
          ) {
    ...
    for (var i = 0; i < this.curQuarterData.length; i++) {
      for (var y = 0; y < this.prevQuarterData.length; y++) {
        if (
          ...
          if (
            (this.prevQuarterData[y].InherentRiskRating == 'High' || this.prevQuarterData[y].InherentRiskRating == 'Severe'
              || this.prevQuarterData[y].InherentRiskRating == 'Catastrophic') &&
            (this.curQuarterData[i].InherentRiskRating == "Medium" || this.curQuarterData[i].InherentRiskRating == 'Low') &&
  ...
  openPopUp(data: any) {
    if (data?.length > 0) {
      const rsca = this.dialog.open(RscaPopupComponent, {
        ...
        data: {
          data: data,
          title: "(Priority >= High) Risk - Inherent Risk Comparison"
        }
      })
    }
    else {
      this.dashboardService.popupInfo("(Priority >= High) Risk - Inherent Risk Comparison", 'No Records Available')
    }
  }
}